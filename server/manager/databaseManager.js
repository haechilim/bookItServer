...
class DatabaseManager {
    constructor() {
        setInterval(() => this.getCategories(), 10800000);
    }
    ...
    getDebates(userId, callback) {
        ...
        ' LEFT JOIN comment AS c ON debate.lastCommentId = c.id' +
    ...
    getMarket(callback) {
        this.query('SELECT market.id, u.id AS uId, u.profileImage AS uProfileImage, u.name AS uName, market.title, market.categoryId, market.status, market.price, market.contents, market.date FROM market LEFT JOIN user AS u ON market.userId = u.id ORDER BY date DESC;', callback);
    }

    writeDebate(userId, title, category, contents, callback) {
        this.query('INSERT INTO debate(userId, bookId, categoryId, title, contents, date) VALUES(' + userId + ', 0, ' + category + ', "' + title + '", "' + contents + '", now());', callback);
    }
    ...
    writeMarket(userId, title, category, status, price, contents, callback) {
        this.query('INSERT INTO market(userId, title, categoryId, status, price, contents, date) VALUES(' + userId + ', "' + title + '", ' + category + ', ' + status + ', ' + price + ', "' + contents + '", now());', callback);
    }
